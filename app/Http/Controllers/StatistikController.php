...
class StatistikController extends Controller
{
    ...
    public function terbit()
    {
        ...
        return view('statistik.terbit', ["title" => "Statistik Perizinan", "heroes" => Hero::all(), "services" => Services::all(), "links" => Link::all(), "linkskm" => Linkskm::all(), 'dataterbitbulans' => $dataterbitbulan, 'dataterbits' => $dataterbit, "tahun" => $tahun, "date1" => $dateid1, "date2" => $dateid2,  "folders" => Folder::limit(6)->get()]);
    }
    ...
    public function nib()
    {
        ...
        $responsenib = Http::get('https://app.magelangkota.go.id/sirekap/reports/oss_rba/rekap_nib_json.php',[
            'send' => $date1 . ';' . $date2
        ]);
        $responseizin = Http::get('https://app.magelangkota.go.id/sirekap/reports/oss_rba/rekap_izin_json.php', [
            'send' => $date1 . ';' . $date2
        ]);
        $dataterbitnib = $responsenib['data'];
        $dataterbitizin = $responseizin['data'];
        $date1x = Carbon::parse($date1);
        $dateid1 = $date1x->locale('id')->isoFormat('Do MMM YYYY');
        $date2x = Carbon::parse($date2);
        $dateid2 = $date2x->locale('id')->isoFormat('Do MMM YYYY');
        return view('statistik.nib', ["title" => "Statistik Perizinan", "heroes" => Hero::all(), "services" => Services::all(), "links" => Link::all(), "linkskm" => Linkskm::all(), 'dataterbitnibs' => $dataterbitnib, 'dataterbitizins' => $dataterbitizin, "tahun" => $tahun, "date1" => $dateid1, "date2" => $dateid2,  "folders" => Folder::limit(6)->get()]);
    }
    //get total per bulan terbit stastistik perizinan non berusaha
    ...
    public function viewgraphterbit(Request $request)
    {
        if ($request->from && $request->to) {
            $date1 = $request->from;
            $date2 = $request->to;
            $tahun = Carbon::createFromFormat('Y-m-d', $date1)->year;
        } else {
            $date1 = date("Y-m-01");
            $date2 = date("Y-m-d");
            $tahun = date("Y");
        ...
        $dateid2 = $date2x->locale('id')->isoFormat('Do MMM YYYY');
        $dataterbitbulan = $responsebulan['data'];
        return view('statistik.terbit', ["title" => "Statistik Perizinan", "heroes" => Hero::all(), "services" => Services::all(), "links" => Link::all(), "linkskm" => Linkskm::all(), "tahun" => $tahun,'dataterbitbulans' => $dataterbitbulan, 'dataterbits' => $dataterbit,  "date1" => $dateid1, "date2" => $dateid2,  "folders" => Folder::limit(6)->get()]);
    }
    //POST terbit stastistik perizinan non berusaha
    ...
    //POST total per bulan terbit stastistik perizinan berusaha
